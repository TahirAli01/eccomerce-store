...
import { Users, Package, ShoppingCart, DollarSign, Check, X, Trash2, Eye } from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'users' | 'products' | 'transactions' | 'categories'>('overview');
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [statsResponse, usersResponse, productsResponse, transactionsResponse, categoriesResponse] = await Promise.all([
  ...
  const toggleBanUser = async (userId: string, isBanned: boolean) => {
    try {
      await axios.put(`http://localhost:3001/api/admin/users/${userId}/ban`, { isBanned });
      setUsers(users.map(user => 
        user.id === userId ? { ...user, isBanned } : user
      ));
      setStats({ ...stats, bannedUsers: stats.bannedUsers + (isBanned ? 1 : -1) });
    } catch (error) {
  ...
  const fetchOrderDetails = async (orderId: string) => {
    try {
      const response = await axios.get(`http://localhost:3001/api/admin/orders/${orderId}`);
  ...
  const handleCategorySubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCategory) {
        ...
        const response = await axios.put(`http://localhost:3001/api/admin/categories/${editingCategory.id}`, categoryForm);
        setCategories(categories.map(cat => cat.id === editingCategory.id ? response.data : cat));
        setFilteredCategories(filteredCategories.map(cat => cat.id === editingCategory.id ? response.data : cat));
      } else {
        // Create new category
        const response = await axios.post('http://localhost:3001/api/admin/categories', categoryForm);
  ...
  const deleteCategory = async (categoryId: string) => {
    if (window.confirm('Are you sure you want to delete this category?')) {
      try {
        await axios.delete(`http://localhost:3001/api/admin/categories/${categoryId}`);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="bg-white rounded-lg shadow-md p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    {stats.bannedUsers > 0 && (
                      <p className="text-sm text-red-600">{stats.bannedUsers} banned</p>
                    )}
                  </div>
                </div>
              </div>
        ...
        {activeTab === 'transactions' && (
          <div className="bg-white rounded-lg shadow-md">
            <div className="p-6 border-b border-gray-200">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <h2 className="text-xl font-semibold text-gray-900 mb-4 sm:mb-0">Transaction Management</h2>
                <div className="relative">
                  <input
                    type="text"
                    ...
                    filteredTransactions.map((transaction) => (
                      <tr key={transaction.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <div className="text-sm text-gray-500">Payment: {transaction.paymentIntentId}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div>
                            ...
                            <div className="text-xs text-gray-400">{transaction.customerRole}</div>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm font-bold text-green-600">£{transaction.total}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
        ...
        {activeTab === 'categories' && (
          <div className="bg-white rounded-lg shadow-md">
            <div className="p-6 border-b border-gray-200">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <h2 className="text-xl font-semibold text-gray-900 mb-4 sm:mb-0">Category Management</h2>
              ...
              <div className="mt-3">
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-blue-50 p-4 rounded-lg">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="text-sm text-blue-900">{selectedOrder.customer.name}</p>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-blue-700">Email</p>
                          <p className="text-sm text-blue-900">{selectedOrder.customer.email}</p>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-blue-700">Role</p>
                          <p className="text-sm text-blue-900">{selectedOrder.customer.role}</p>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-blue-700">Customer ID</p>
                          <p className="text-sm text-blue-900">{selectedOrder.customer.id}</p>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="bg-green-50 p-4 rounded-lg">
                    <h4 className="text-sm font-medium text-green-900 mb-2">Shipping Address</h4>
                    <div className="space-y-1">
                      <p className="text-sm text-green-900">{selectedOrder.shippingAddress.fullName}</p>
                      <p className="text-sm text-green-900">{selectedOrder.shippingAddress.address}</p>
                      <p className="text-sm text-green-900">{selectedOrder.shippingAddress.city}, {selectedOrder.shippingAddress.country}</p>
                    </div>
                  </div>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {item.productDetails && (
                              <div className="mt-2 text-xs text-gray-500">
                                ...
                                <p>Status: {item.productDetails.isActive ? 'Active' : 'Inactive'}</p>
                              </div>
                            )}
                          </div>
                        </div>
                  ...
                  <div className="bg-purple-50 p-4 rounded-lg">
                    <h4 className="text-sm font-medium text-purple-900 mb-2">Payment Information</h4>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-sm font-medium text-purple-700">Payment Intent ID</p>
                        <p className="text-sm text-purple-900 font-mono">{selectedOrder.paymentIntentId}</p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-purple-700">Payment Status</p>
                        <p className="text-sm text-purple-900">{selectedOrder.status}</p>
                      </div>
                    </div>
                  </div>
                </div>
